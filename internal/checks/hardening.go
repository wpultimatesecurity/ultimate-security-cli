...
func init() {
	Register(Simple{
		Meta: Meta{
			...
			// Exposure signal, not a missing control: XML-RPC is legitimate on
			// many sites, so it is reported without a universal score
			// deduction. Projects that treat it as a hard requirement can
			// raise it with severity_overrides.
...
func runXMLRPC(ctx *Context) []Finding {
	m := Meta{ID: "XMLRPC_ENABLED", Title: "XML-RPC is enabled", Category: CatHardening,
